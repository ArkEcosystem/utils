...
export class DotEnv {
    public parse(contents: string): Record<string, Primitive> {
    ...
    public parseFile(file: string): Record<string, Primitive> {
        return this.parse(readFileSync(file).toString());
    }

    public stringify(pairs: object): string {
